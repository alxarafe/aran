...
class AlixarView extends \Alixar\Base\AlixarBasicView
{
     ...
     *
     * @param 	string 	$this->head				Optionnal head lines
     * @param 	string 	$this->title				HTML title
     * @param	string	$this->help_url			Url links to help page
     * 		                            	Syntax is: For a wiki page: EN:EnglishPage|FR:FrenchPage|ES:SpanishPage
     *                                  	For other external page: http://server/url
     * @param	string	$this->target				Target to use on links
     * @param 	int    	$this->disablejs			More content into html header
     * @param 	int    	$this->disablehead		More content into html header
     * @param 	array  	$this->arrayofjs			Array of complementary js files
     * @param 	array  	$this->arrayofcss			Array of complementary css files
     * @param	string	$this->morequerystring	Query string to add to the link "print" to get same parameters (use only if autodetect fails)
     * @param   string  $this->morecssonbody      More CSS on body tag.
     * @param	string	$this->replacemainareaby	Replace call to main_area() by a print of this string
     * @return	void
     */
    function llxHeader($_head = '', $_title = '', $_help_url = '', $_target = '', $_disablejs = 0, $_disablehead = 0, $_arrayofjs = '', $_arrayofcss = '', $_morequerystring = '', $_morecssonbody = '', $_replacemainareaby = '')
    {
        $this->head = $_head;
        $this->title = $_title;
        $this->help_url = $_help_url;
        $this->target = $_target;
        $this->disablejs = $_disablejs;
        $this->disablehead = $_disablehead;
        $this->arrayofjs = $_arrayofjs;
        $this->arrayofcss = $_arrayofcss;
        $this->morequerystring = $_morequerystring;
        $this->morecssonbody = $_morecssonbody;
        $this->replacemainareaby = $_replacemainareaby;
...
// top menu and left menu area
        if (empty(Globals::$conf->dol_hide_topmenu) || GETPOST('dol_invisible_topmenu', 'int')) {
            $this->top_menu(/* $this->head, $this->title, $this->target, $this->disablejs, $this->disablehead, $this->arrayofjs, $this->arrayofcss, $this->morequerystring, $this->help_url */);
        }
...
// main area
        if ($this->replacemainareaby) {
            print $this->replacemainareaby;
            return;
        }
        $this->main_area($this->title);
    }

    /**
     *  Show an HTML header + a BODY + The top menu bar
     *
     *  @param      string	$this->head    			Lines in the HEAD
     *  @param      string	$this->title   			Title of web page
     *  @param      string	$this->target  			Target to use in menu links (Example: '' or '_top')
     * 	@param		int		$this->disablejs			Do not output links to js (Ex: qd fonction utilisee par sous formulaire Ajax)
     * 	@param		int		$this->disablehead		Do not output head section
     * 	@param		array	$this->arrayofjs			Array of js files to add in header
     * 	@param		array	$this->arrayofcss			Array of css files to add in header
     *  @param		string	$this->morequerystring	Query string to add to the link "print" to get same parameters (use only if autodetect fails)
     ...
     *  @return		void
     */
    function top_menu(/* $this->head, $this->title = '', $this->target = '', $this->disablejs = 0, $this->disablehead = 0, $this->arrayofjs = '', $this->arrayofcss = '', $this->morequerystring = '', $helppagename = '' */)
    {
...
// Get helpbaseurl, helppage and mode from helppagename and langs
                $arrayres = $this->getHelpParamFor($helppagename, Globals::$langs);
...
// Logout link
     ...
     *  @return	void
     */
    function left_menu($menu_array_before, /* $helppagename = '', */ $notused = '', $menu_array_after = '', $leftmenuwithoutmainarea = 0, /* $this->title = '', */ $acceptdelayedhtml = 0)
    {
        $searchform = '';
        $bookmarks = '';

        $helppagename = $this->help_url;
        ...
        if (empty(Globals::$conf->dol_hide_leftmenu) && (!defined('NOREQUIREMENU') || !constant('NOREQUIREMENU'))) {
            ...
            $arrayresult = AjaxLib::selectSearchBox();
...
// Execute hook printLeftBlock
     ...
     *  Begin main area
     *
     *  @param	string	$this->title		Title
     *  @return	void
     */
    function main_area(/* $this->title = '' */)
    {
...
// If WIKI URL
     ...
     *  @param  string	$this->title              Title search area
     ...
     *  @return	string
     */
    function printSearchForm($urlaction, $urlobject, $this_title, $htmlmorecss, $htmlinputname, $accesskey = '', $prefhtmlinputname = '', $img = '', $showtitlebefore = 0)
    {
