...
/*
  ...
  //header("Location: " . DOL_URL_ROOT . "/admin/index.php?mainmenu=home&leftmenu=setup&mesg=setupnotcomplete");
  echo "<p>Total modules: " . count($conf->modules) . "</p>";
  var_dump($conf->global->MAIN_MIN_NB_ENABLED_MODULE_FOR_WARNING);
  die('2');
  header('Location: ' . BASE_URI . '?controller=admin&method=index&mainmenu=home&leftmenu=setup&mesg=setupnotcomplete');
  exit;
  }
...
if (!empty($conf->global->MAIN_USE_METEO_WITH_PERCENTAGE) && !empty($totaltodo))
    $totallate = round($totallate / $totaltodo * 100, 2);
//var_dump($totallate);
$boxwork = '';
