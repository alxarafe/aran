...
if (!$error && $db->connected && $action == "set") {
    ...
    // Show title of step
    print '<h3><img class="valigntextbottom" src="' . DOL_BASE_URI . '/theme/common/octicons/build/svg/gear.svg" width="20" alt="Configuration"> ' . $langs->trans("ConfigurationFile") . '</h3>';
    ...
    if (!$error) {
            ...
            if (!$error) {
                if ($db->connected) {
                    ...
                    if ($result > 0 && $resultbis > 0) {
                        ...
                        if ($db->errno() == 'DB_ERROR_RECORD_ALREADY_EXISTS' || $db->errno() == 'DB_ERROR_KEY_NAME_ALREADY_EXISTS' || $db->errno() == 'DB_ERROR_USER_ALREADY_EXISTS') {
                    ...
                    print $dolibarr_main_db_user;
                    print '</td>';
                    print '<td><img src="' . DOL_BASE_URI . '/theme/eldy/img/error.png" alt="Error"></td>';
                    print '</tr>';
        ...
        if (!$error) {
            ...
            if ($db->connected) {
                ...
                print "</td><td>";
                print '<img src="' . DOL_BASE_URI . '/theme/eldy/img/tick.png" alt="Ok">';
                print "</td></tr>";
                ...
                if ($db->database_selected) {
                    ...
                    print $dolibarr_main_db_name;
                    print '</td><td>';
                    print '<img src="' . DOL_BASE_URI . '/theme/eldy/img/error.png" alt="Error">';
                    print "</td></tr>";
...
function write_conf_file($conffile)
{
    ...
    if ($fp) {
        ...
        fputs($fp, "\n");
        if (empty($force_dolibarr_lib_PHPEXCEL_PATH)) {
            fputs($fp, '//');
        ...
        fputs($fp, "\n");
        if (empty($force_dolibarr_js_JQUERY_FLOT)) {
            fputs($fp, '//');
            $force_dolibarr_js_JQUERY_FLOT = '';
        }
