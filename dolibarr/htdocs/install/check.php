...
print '<h3><img class="valigntextbottom" src="' . DOL_BASE_URI . '/theme/common/octicons/build/svg/gear.svg" width="20" alt="Database"> ' . $langs->trans("MiscellaneousChecks") . ":</h3>\n";
...
if (!empty($useragent)) {
    ...
    if ($browsername == 'ie' && $browserversion < 7)
        print '<img src="' . DOL_BASE_URI . '/theme/eldy/img/warning.png" alt="Error"> ' . $langs->trans("WarningBrowserTooOld") . "<br>\n";
}
...
if (versioncompare(versionphparray(), $arrayphpminversionerror) < 0) {        // Minimum to use (error if lower)
    print '<img src="' . DOL_BASE_URI . '/theme/eldy/img/error.png" alt="Error"> ' . $langs->trans("ErrorPHPVersionTooLow", versiontostring($arrayphpminversionerror));
    ...
    print '<img src="' . DOL_BASE_URI . '/theme/eldy/img/warning.png" alt="Error"> ' . $langs->trans("ErrorPHPVersionTooLow", versiontostring($arrayphpminversionwarning));
    $checksok = 0; // 0=error, 1=warning
} else {
    print '<img src="' . DOL_BASE_URI . '/theme/eldy/img/tick.png" alt="Ok"> ' . $langs->trans("PHPVersion") . " " . versiontostring(versionphparray());
}
...
if (!isset($_GET["testget"]) && !isset($_POST["testpost"])) {
    ...
    print ' (<a href="?controller=install&method=check&testget=ok">' . $langs->trans("Recheck") . '</a>)';
    print "<br>\n";
    $checksok = 0;
} else {
...
if (!function_exists("imagecreate")) {
    $langs->load("errors");
    print '<img src="' . DOL_BASE_URI . '/theme/eldy/img/warning.png" alt="Error"> ' . $langs->trans("ErrorPHPDoesNotSupportGD") . "<br>\n";
...
if (!function_exists("curl_init")) {
    $langs->load("errors");
    print '<img src="' . DOL_BASE_URI . '/theme/eldy/img/warning.png" alt="Error"> ' . $langs->trans("ErrorPHPDoesNotSupportCurl") . "<br>\n";
    // $checksok=0;		// If image ko, just warning. So check must still be 1 (otherwise no way to install)
} else {
...
if (!function_exists("utf8_encode")) {
    $langs->load("errors");
    print '<img src="' . DOL_BASE_URI . '/theme/eldy/img/warning.png" alt="Error"> ' . $langs->trans("ErrorPHPDoesNotSupportUTF8") . "<br>\n";
    // $checksok=0;		// If image ko, just warning. So check must still be 1 (otherwise no way to install)
} else {
...
if (!file_exists($conffile)) {
    print '<img src="' . DOL_BASE_URI . '/theme/eldy/img/error.png" alt="Error"> ' . $langs->trans("ConfFileDoesNotExistsAndCouldNotBeCreated", $conffiletoshow);
    print "<br><br>";
    ...
    if (dol_is_dir($conffile)) {
        print '<img src="' . DOL_BASE_URI . '/theme/eldy/img/error.png" alt="Warning"> ' . $langs->trans("ConfFileMustBeAFileNotADir", $conffiletoshow);
    ...
    elseif (!is_writable($conffile)) {
        if ($confexists) {
            print '<img src="' . DOL_BASE_URI . '/theme/eldy/img/tick.png" alt="Ok"> ' . $langs->trans("ConfFileExists", $conffiletoshow);
        } else {
            print '<img src="' . DOL_BASE_URI . '/theme/eldy/img/tick.png" alt="Ok"> ' . $langs->trans("ConfFileCouldBeCreated", $conffiletoshow);
        }
        print "<br>";
        print '<img src="' . DOL_BASE_URI . '/theme/eldy/img/tick.png" alt="Warning"> ' . $langs->trans("ConfFileIsNotWritable", $conffiletoshow);
        print "<br>\n";
    ...
    // File exists and can be modified
        ...
        print "<br>";
        print '<img src="' . DOL_BASE_URI . '/theme/eldy/img/tick.png" alt="Ok"> ' . $langs->trans("ConfFileIsWritable", $conffiletoshow);
        print "<br>\n";
    ...
    if ($checksok) {
        ...
        if ($allowinstall) {
            ...
            $choice .= '<a class="button" href="?controller=install&method=fileconf&selectlang=' . $setuplang . '">' . $langs->trans("Start") . '</a>';
        } else {
        ...
        if (count($notavailable_choices)) {
            ...
            print '<img src="' . DOL_BASE_URI . '/theme/eldy/img/1downarrow.png">';
            print '</div>';
...
$("div#AShowChoices a").click(function() {
    ...
    if ($("div#navail_choices").css("display") == "none") {
        ...
        $(this).parent().children("img").attr("src", "' . DOL_BASE_URI . '/theme/eldy/img/1downarrow.png");
    } else {
        $(this).text("' . $langs->trans('HideNotAvailableOptions') . '");
        $(this).parent().children("img").attr("src", "' . DOL_BASE_URI . '/theme/eldy/img/1uparrow.png");
    }
