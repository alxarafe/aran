...
if ($action == "set") {
    print '<h3><img class="valigntextbottom" src="' . DOL_BASE_URI . '/theme/common/octicons/build/svg/database.svg" width="20" alt="Database"> ' . $langs->trans("Database") . '</h3>';
    ...
    if ($db->connected) {
        print "<tr><td>";
        print $langs->trans("ServerConnection") . " : " . $conf->db->host . '</td><td><img src="' . DOL_BASE_URI . '/theme/eldy/img/tick.png" alt="Ok"></td></tr>';
        $ok = 1;
    } else {
        print "<tr><td>Failed to connect to server : " . $conf->db->host . '</td><td><img src="' . DOL_BASE_URI . '/theme/eldy/img/error.png" alt="Error"></td></tr>';
    }
    ...
    if ($ok) {
        if ($db->database_selected) {
            ...
            print "<tr><td>Failed to select database " . $conf->db->name . '</td><td><img src="' . DOL_BASE_URI . '/theme/eldy/img/error.png" alt="Error"></td></tr>';
            $ok = 0;
        }
    }
    ...
    if ($ok && $createtables) {
        ...
        if ($tablefound) {
            if ($error == 0) {
                print '<tr><td>';
                print $langs->trans("TablesAndPrimaryKeysCreation") . '</td><td><img src="' . DOL_BASE_URI . '/theme/eldy/img/tick.png" alt="Ok"></td></tr>';
                $ok = 1;
            }
        } else {
            print '<tr><td>' . $langs->trans("ErrorFailedToFindSomeFiles", $dir) . '</td><td><img src="' . DOL_BASE_URI . '/theme/eldy/img/error.png" alt="Error"></td></tr>';
    ...
    if ($ok && $createkeys) {
        ...
        if ($tablefound && $error == 0) {
            print '<tr><td>';
            print $langs->trans("OtherKeysCreation") . '</td><td><img src="' . DOL_BASE_URI . '/theme/eldy/img/tick.png" alt="Ok"></td></tr>';
            $okkeys = 1;
        }
    }
    ...
    if ($ok && $createfunctions) {
        ...
        if (file_exists($dir . $file)) {
            ...
            foreach ($listesql as $buffer) {
                ...
                if ($buffer) {
                    ...
                    if ($resql) {
                        ...
                        if ($db->errno() == 'DB_ERROR_RECORD_ALREADY_EXISTS' || $db->errno() == 'DB_ERROR_KEY_NAME_ALREADY_EXISTS') {
                            //print "Insert line : ".$buffer."<br>\n";
                        } else {
                            $ok = 0;
            ...
            if ($ok) {
                print '<td><img src="' . DOL_BASE_URI . '/theme/eldy/img/tick.png" alt="Ok"></td></tr>';
            } else {
                print '<td><img src="' . DOL_BASE_URI . '/theme/eldy/img/error.png" alt="Error"></td></tr>';
                $ok = 1;
            }
        }
    }
