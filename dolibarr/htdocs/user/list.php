...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) {
    $sql .= " LEFT JOIN " . MAIN_DB_PREFIX . "user_extrafields as ef on (u.rowid = ef.fk_object)";
}
...
if ($search_supervisor > 0) {
    $sql .= " AND u.fk_user IN (" . $db->escape($search_supervisor) . ")";
}
...
if ($search_gender != '' && $search_gender != '-1') {
    $sql .= " AND u.gender = '" . $search_gender . "'";
}
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $sall) {
    ...
    header("Location: " . BASE_URI . '?controller=user&method=card&id=' . $id);
    exit;
}
...
if ($search_gender != '') {
    $param .= "&search_gender=" . $search_gender;
}
if ($search_employee != '') {
    $param .= "&search_employee=" . $search_employee;
}
if ($search_accountancy_code != '') {
...
if ($search_supervisor > 0) {
    $param .= "&search_supervisor=" . $search_supervisor;
}
if ($search_statut != '') {
...
if ($canadduser) {
    ...
    $newcardbutton = '<a class="butActionNew" href="' . BASE_URI . '?controller=user&method=card&action=create' . ($mode == 'employee' ? '&employee=1' : '') . '&leftmenu="><span class="valignmiddle">' . $langs->trans('NewUser') . '</span>';
...
if (!empty($arrayfields['u.datelastlogin']['checked'])) {
    print_liste_field_titre("LastConnexion", $_SERVER['PHP_SELF'], "u.datelastlogin", $param, "", 'align="center"', $sortfield, $sortorder);
}
if (!empty($arrayfields['u.datepreviouslogin']['checked'])) {
    print_liste_field_titre("PreviousConnexion", $_SERVER['PHP_SELF'], "u.datepreviouslogin", $param, "", 'align="center"', $sortfield, $sortorder);
}
// Extra fields
...
print $hookmanager->resPrint;
if (!empty($arrayfields['u.datec']['checked'])) {
    print_liste_field_titre("DateCreationShort", $_SERVER["PHP_SELF"], "u.datec", "", $param, 'align="center" class="nowrap"', $sortfield, $sortorder);
}
if (!empty($arrayfields['u.tms']['checked'])) {
    print_liste_field_titre("DateModificationShort", $_SERVER["PHP_SELF"], "u.tms", "", $param, 'align="center" class="nowrap"', $sortfield, $sortorder);
}
if (!empty($arrayfields['u.statut']['checked'])) {
    print_liste_field_titre("Status", $_SERVER["PHP_SELF"], "u.statut", "", $param, 'align="center"', $sortfield, $sortorder);
}
