 ...
 * Copyright (C) 2018-2018  Alxarafe                <info@alxarafe.com>
 *
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && ($search_all != '' || $search_cti != '') && $action != 'list') {
    ...
    if (!empty($conf->global->SOCIETE_ON_SEARCH_AND_LIST_GO_ON_CUSTOMER_OR_SUPPLIER_CARD)) {
        if ($obj->client > 0) {
            ...
            header("Location: " . BASE_URI . '?controller=comm&method=card&socid=' . $id);
            exit;
        }
        if ($obj->fournisseur > 0) {
            //header("Location: ".DOL_URL_ROOT.'/fourn/card.php?socid='.$id);
            header("Location: " . BASE_URI . '?controller=fourn&method=card&socid=' . $id);
            exit;
        }
    }

    //header("Location: ".DOL_URL_ROOT.'/societe/card.php?socid='.$id);
    header("Location: " . BASE_URI . '?controller=societe&method=card&socid=' . $id);
...
if ($user->rights->societe->creer) {
    ...
    $newcardbutton = '<a class="butActionNew" href="' . BASE_URI . '?controller=societe&method=card&action=create' . $typefilter . '"><span class="valignmiddle">' . $langs->trans($label) . '</span>';
