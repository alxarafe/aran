                ...
                if ($result >= 0 && !$error) {
                    ...
                    if (!empty($backtopage)) {
                        ...
                        if (($object->client == 1 || $object->client == 3) && empty($conf->global->SOCIETE_DISABLE_CUSTOMERS)) {
                            ...
                            $url = BASE_URI . "?controller=comm&method=card&socid=" . $object->id;
                        } else if ($object->fournisseur == 1) {
                            // $url = DOL_URL_ROOT . "/fourn/card.php?socid=" . $object->id;
                            $url = BASE_URI . "?controller=fourn&method=card&socid=" . $object->id;
                        }
...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
    ...
    if ($action == 'create') {
        ...
        print '<form enctype="multipart/form-data" action="' . '?controller=' . 'societe' . '&method=' . 'card' . '" method="post" name="formsoc" autocomplete="off">';  // Chrome ignor autocomplete
        ...
        if ((!empty($conf->fournisseur->enabled) && !empty($user->rights->fournisseur->lire)) || (!empty($conf->supplier_proposal->enabled) && !empty($user->rights->supplier_proposal->lire))) {
            // Supplier
            print '<tr>';
