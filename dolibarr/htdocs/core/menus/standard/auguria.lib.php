    ...
    if ($conf->banque->enabled && $user->rights->banque->lire && $mainmenu == 'bank') { // Entry for each bank account
        ...
        if ($resql) {
            ...
            if ($numr > 0) {
                ...
                $newmenu->add('?controller=compta/bank&list', $langs->trans("BankAccounts"), 0, $user->rights->banque->lire);
            }

            while ($i < $numr) {
                $objp = $db->fetch_object($resql);
                //$newmenu->add('/compta/bank/card.php?id=' . $objp->rowid, $objp->label, 1, $user->rights->banque->lire);
                $newmenu->add('?controller=compta/bank&method=card&id=' . $objp->rowid, $objp->label, 1, $user->rights->banque->lire);
                if ($objp->rappro && $objp->courant != Account::TYPE_CASH && empty($objp->clos)) {  // If not cash account and not closed and can be reconciliate
                    ...
                    $newmenu->add('?controller=compta/bank&method=bankentries_list&id=' . $objp->rowid, $langs->trans("Conciliate"), 2, $user->rights->banque->consolidate);
                }
                $i++;
            }
        } else
                ...
                while ($i < $numr) {
                    ...
                    if ($nature) {
                        if ($usemenuhider || empty($leftmenu) || preg_match('/accountancy/', $leftmenu)) {
                            ...
                            $newmenu->add('?controller=accountancy/journal&method=' . $nature . 'journal&mainmenu=accountancy&leftmenu=accountancy_journal&id_journal=' . $objp->rowid, dol_trunc($objp->label, 25), 2, $user->rights->accounting->comptarapport->lire);
                        }
                    }
                    $i++;
                }
    ...
    if ($conf->ftp->enabled && $mainmenu == 'ftp') { // Entry for FTP
        ...
        while ($i <= $MAXFTP) {
            ...
            if (!empty($conf->global->$paramkey)) {
                ...
                $link = "?controller=ftp&method=index&idmenu=" . $_SESSION["idmenu"] . "&numero_ftp=" . $i;
