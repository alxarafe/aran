...
function societe_prepare_head(Societe $object)
{
    ...
    $head[$h][0] = BASE_URI . '?controller=societe&method=card&socid=' . $object->id;
    ...
    if (empty($conf->global->MAIN_SUPPORT_SHARED_CONTACT_BETWEEN_THIRDPARTIES))
    {
	    if (empty($conf->global->MAIN_DISABLE_CONTACTS_TAB))
		{
            ...
            $head[$h][0] = BASE_URI . '?controller=societe&method=contact&socid=' . $object->id;
    ...
    else
	{
        ...
        $head[$h][0] = BASE_URI . '?controller=societe&method=societecontact&socid=' . $object->id;
        $nbContact = count($object->liste_contact(-1,'internal')) + count($object->liste_contact(-1,'external'));
		$head[$h][1] = $langs->trans("ContactsAddresses");
		if ($nbContact > 0) $head[$h][1].= ' <span class="badge">'.$nbContact.'</span>';
		$head[$h][2] = 'contact';
    ...
    if ($object->client==1 || $object->client==2 || $object->client==3)
    {
        ...
        $head[$h][0] = BASE_URI . '?controller=comm&method=card&socid=' . $object->id;
        $head[$h][1] = '';
        ...
        if (! empty($conf->global->PRODUIT_CUSTOMER_PRICES))
        {
            ...
            $head[$h][0] = BASE_URI . '?controller=societe&method=price&socid=' . $object->id;
    ...
    if (! empty($conf->fournisseur->enabled) && $object->fournisseur && ! empty($user->rights->fournisseur->lire))
    {
        ...
        $head[$h][0] = BASE_URI . '?controller=fourn&method=card&socid=' . $object->id;
    ...
    if (! empty($conf->projet->enabled) && (!empty($user->rights->projet->lire) ))
    {
        ...
        $head[$h][0] = BASE_URI . '?controller=societe&method=project&socid=' . $object->id;
	...
	if (! empty($conf->resource->enabled) && ! empty($conf->global->RESOURCE_ON_THIRDPARTIES))
	{
        ...
        $head[$h][0] = BASE_URI . '?controller=resource&method=element_resource&element=societe&element_id=' . $object->id;
	...
	if (! empty($conf->global->ACCOUNTING_ENABLE_LETTERING))
	{
		...
		if (! empty($conf->accounting->enabled) && $object->client>0)
		{
            ...
            $head[$h][0] = BASE_URI . '?controller=accountancy/bookkeeping&method=thirdparty_lettering_customer&socid=' . $object->id;
		...
		if (! empty($conf->accounting->enabled) && $object->fournisseur>0)
		{
            ...
            $head[$h][0] = BASE_URI . '?controller=accountancy/bookkeeping&method=thirdparty_lettering_supplier&socid=' . $object->id;
    ...
    if (! empty($conf->commande->enabled) || ! empty($conf->propal->enabled) || ! empty($conf->facture->enabled) || ! empty($conf->ficheinter->enabled) || ! empty($conf->fournisseur->enabled))
    {
        ...
        $head[$h][0] = BASE_URI . '?controller=societe&method=consumption&socid=' . $object->id;
    ...
    if (empty($conf->global->SOCIETE_DISABLE_BANKACCOUNT))
    {
        ...
        $head[$h][0] = BASE_URI . '?controller=societe&method=paymentmodes&socid=' . $object->id;
        $head[$h][1] = $title;
    ...
    if (!empty($conf->website->enabled) && (!empty($conf->global->WEBSITE_USE_WEBSITE_ACCOUNTS)) && (!empty($user->rights->societe->lire))) {
        ...
        $head[$h][0] = BASE_URI . '?controller=societe&method=website&id=' . $object->id;
    ...
    if ($user->societe_id == 0) {
        ...
        if (! empty($conf->notification->enabled))
        {
            ...
            $head[$h][0] = BASE_URI . '?controller=societe/notify&method=card&socid=' . $object->id;
        ...
        $head[$h][0] = BASE_URI . '?controller=societe&method=note&id=' . $object->id;
        ...
        $head[$h][0] = BASE_URI . '?controller=societe&method=document&socid=' . $object->id;
    ...
    $head[$h][0] = BASE_URI . '?controller=societe&method=agenda&socid=' . $object->id;
