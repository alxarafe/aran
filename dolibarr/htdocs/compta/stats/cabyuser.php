...
print_liste_field_titre(
    $langs->trans("User"), $_SERVER["PHP_SELF"], "name", "", $paramslink, "", $sortfield, $sortorder
);
if ($modecompta == 'CREANCES-DETTES') {
    print_liste_field_titre(
        $langs->trans('AmountHT'), $_SERVER["PHP_SELF"], "amount_ht", "", $paramslink, 'align="right"', $sortfield, $sortorder
    );
} else {
    print_liste_field_titre('');
}
print_liste_field_titre(
    $langs->trans("AmountTTC"), $_SERVER["PHP_SELF"], "amount_ttc", "", $paramslink, 'align="right"', $sortfield, $sortorder
);
print_liste_field_titre(
    $langs->trans("Percentage"), $_SERVER["PHP_SELF"], "amount_ttc", "", $paramslink, 'align="right"', $sortfield, $sortorder
);
print_liste_field_titre(
    $langs->trans("OtherStatistics"), $_SERVER["PHP_SELF"], "", "", "", 'align="center" width="20%"'
);
print "</tr>\n";
...
if (count($amount)) {
    ...
    foreach ($arrayforsort as $key => $value) {
        ...
        if ($key >= 0) {
            ...
            $linkname = '<a href="' . DOL_BASE_URI . '?controller=user&methos=card&id=' . $key . '">' . img_object($langs->trans("ShowUser"), 'user') . ' ' . $fullname . '</a>';
        } else {
        ...
        if ($modecompta != 'CREANCES-DETTES') {
            if ($key > 0) {
                ...
                print '<a href="' . DOL_BASE_URI . '?controller=compta/paiement&method=list&userid=' . $key . '">';
            } else {
                // print '<a href="' . DOL_URL_ROOT . '/compta/paiement/list.php?userid=-1">';
                print '<a href="' . DOL_BASE_URI . '?controller=compta/paiement&method=list&userid=-1">';
            }
        } else {
            if ($key > 0) {
                // print '<a href="' . DOL_URL_ROOT . '/compta/facture/list.php?userid=' . $key . '">';
                print '<a href="' . DOL_BASE_URI . '?controller=compta/facture&method=list&userid=' . $key . '">';
            } else {
                print '<a href="#">';
            }
        ...
        if (!empty($conf->propal->enabled) && $key > 0) {
            ...
            print '&nbsp;<a href="' . DOL_BASE_URI . '?controller=comm/propal/stats&method=index&userid=' . $key . '">' . img_picto($langs->trans("ProposalStats"), "stats") . '</a>&nbsp;';
        }
        if (!empty($conf->commande->enabled) && $key > 0) {
            ...
            print '&nbsp;<a href="' . DOL_BASE_URI . '?controller=commande/stats&method=index&userid=' . $key . '">' . img_picto($langs->trans("OrderStats"), "stats") . '</a>&nbsp;';
        }
        if (!empty($conf->facture->enabled) && $key > 0) {
            ...
            print '&nbsp;<a href="' . DOL_BASE_URI . '?controller=compta/facture/stats&method=index&userid=' . $key . '">' . img_picto($langs->trans("InvoiceStats"), "stats") . '</a>&nbsp;';
        }
        print '</td>';
        print "</tr>\n";
        $i++;
