...
if ($object->id > 0) {
    ...
    if ($user->rights->societe->creer && !$user->societe_id > 0) {
        ...
        print '<a href="' . BASE_PATH . '?controller=comm&method=remise&id=' . $object->id . '">' . img_edit($langs->trans("Modify")) . '</a>';
    }
    print '</td></tr></table>';
    ...
    print '</td><td>' . ($object->remise_supplier_percent ? '<a href="' . BASE_PATH . '?controller=comm&method=remise&id=' . $object->id . '">' . $object->remise_supplier_percent . '%</a>' : '') . '</td>';
    print '</tr>';
    ...
    if ($user->rights->societe->creer && !$user->societe_id > 0) {
        ...
        print '<a href="' . BASE_PATH . '?controller=comm&method=remx&id=' . $object->id . '&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?socid=' . $object->id) . '">' . img_edit($langs->trans("Modify")) . '</a>';
    }
    print '</td></tr></table>';
    print '</td>';
    print '<td>';
    ...
    //print '<a href="' . DOL_URL_ROOT . '/comm/remx.php?id=' . $object->id . '&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?socid=' . $object->id) . '">' . price($amount_discount, 1, $langs, 1, -1, -1, $conf->currency) . '</a>';
        print '<a href="' . BASE_PATH . '?controller=comm&method=remx&id=' . $object->id . '&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?socid=' . $object->id) . '">' . price($amount_discount, 1, $langs, 1, -1, -1, $conf->currency) . '</a>';
    ...
    if ($conf->supplier_proposal->enabled) {
        ...
        $link = BASE_PATH . '?controller=supplier_proposal&method=list&socid=' . $object->id;
        $icon = 'bill';
        if ($link)
