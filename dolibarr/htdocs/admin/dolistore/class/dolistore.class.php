...
include_once DOL_BASE_PATH . '/core/lib/admin.lib.php';
include_once DOL_BASE_PATH . '/admin/dolistore/class/PSWebServiceLibrary.class.php';
...
class Dolistore
{
    ...
    function __construct($debug = false)
    {
        ...
        $this->url = DOL_BASE_URI . '/admin/modules.php?mode=marketplace';
        ...
        $langtmp = explode('_', Globals::$langs->defaultlang);
        $lang = $langtmp[0];
    ...
    function get_products()
    {
        ...
        foreach ($this->products as $product) {
            ...
            if ($product->id_default_image != '') {
                $image_url = DOL_BASE_URI . '/admin/dolistore/ajax/image.php?id_product=' . $product->id . '&id_image=' . $product->id_default_image;
                ...
                $images = '<img src="' . DOL_BASE_URI . '/admin/dolistore/img/NoImageAvailable.png" />';
            }

            // free or pay ?
            if ($product->price > 0) {
                $price = '<h3>' . price(round((float) $product->price * $this->vat_rate, 2)) . '&nbsp;&euro;</h3>';
                $download_link = '<a target="_blank" href="' . $this->shop_url . $product->id . '"><img width="32" src="' . DOL_BASE_URI . '/admin/dolistore/img/follow.png" /></a>';
            } else {
                $price = '<h3>' . $langs->trans('Free') . '</h3>';
                $download_link = '<a target="_blank" href="' . $this->shop_url . $product->id . '"><img width="32" src="' . DOL_BASE_URI . '/admin/dolistore/img/Download-128.png" /></a>';
                $download_link .= '<br><br><a target="_blank" href="' . $this->shop_url . $product->id . '"><img width="32" src="' . DOL_BASE_URI . '/admin/dolistore/img/follow.png" /></a>';
            }
            ...
            if ($this->version_compare($product->dolibarr_min, DOL_VERSION) <= 0) {
                if ($this->version_compare($product->dolibarr_max, DOL_VERSION) >= 0) {
                    //compatible
                    $version = '<span class="compatible">' . $langs->trans('CompatibleUpTo', $product->dolibarr_max, $product->dolibarr_min, $product->dolibarr_max) . '</span>';
                    $compatible = '';
                } else {
                    //never compatible, module expired
                    $version = '<span class="notcompatible">' . $langs->trans('NotCompatible', DOL_VERSION, $product->dolibarr_min, $product->dolibarr_max) . '</span>';
                    $compatible = 'NotCompatible';
                }
            } else {
                //need update
                $version = '<span class="compatibleafterupdate">' . $langs->trans('CompatibleAfterUpdate', DOL_VERSION, $product->dolibarr_min, $product->dolibarr_max) . '</span>';
                $compatible = 'NotCompatible';
            }
