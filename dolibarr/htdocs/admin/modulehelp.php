...
$head[$h][0] = BASE_URI . "?controller=admin&method=modulehelp&id=" . $id . '&mode=desc';
$head[$h][1] = $langs->trans("Description");
$head[$h][2] = 'desc';
$h++;

//$head[$h][0] = DOL_URL_ROOT . "/admin/modulehelp.php?id=" . $id . '&mode=feature';
$head[$h][0] = BASE_URI . "?controller=admin&method=modulehelp&id=" . $id . '&mode=feature';
...
if ($objMod->isCoreOrExternalModule() == 'external') {
    ...
    $head[$h][0] = BASE_URI . "?controller=admin&method=modulehelp&id=" . $id . '&mode=changelog';
...
if ($mode == 'desc') {
    ...
    if ($objMod->isCoreOrExternalModule() == 'external') {
        ...
        if ($objMod->editor_name != 'dolibarr') {
            $textexternal .= '<br><strong>' . $langs->trans("Publisher") . ':</strong> ' . (empty($objMod->editor_name) ? $langs->trans("Unknown") : $objMod->editor_name);
        }
        ...
        if (!empty($objMod->editor_url) && !preg_match('/dolibarr\.org/i', $objMod->editor_url)) {
            $textexternal .= '<br><strong>' . $langs->trans("Url") . ':</strong> <a href="' . $editor_url . '" target="_blank">' . $objMod->editor_url . '</a>';
        }
        $text .= $textexternal;
        $text .= '<br>';
    } else {
