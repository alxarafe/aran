    ...
    if ($action == 'add' && $user->rights->societe->contact->creer) {
        ...
        if (!$error && $id > 0) {
            ...
            if (!empty($backtopage)) {
                ...
                // $url = 'card.php?id=' . $id;
                $url = BASE_URI . '?controller=contact&method=card&id=' . $id;
            }
            header("Location: " . $url);
            exit;
        } else {
...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
    ...
    if ($user->rights->societe->contact->creer) {
        if ($action == 'create') {
            ...
            print '<form method="post" name="formsoc" href="' . BASE_URI . '?controller=contact&method=card">';
            ...
            print '<form enctype="multipart/form-data" method="post" href="' . BASE_URI . '?controller=contact&method=card&id=' . $id . '" name="formsoc">';
            print '<input type="hidden" name="token" value="' . $_SESSION['newtoken'] . '">';
