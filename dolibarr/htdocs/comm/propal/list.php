...
if ($search_montant_vat != '') {
    $sql .= natural_search("p.tva", $search_montant_vat, 1);
}
if ($search_montant_ttc != '') {
    $sql .= natural_search("p.total", $search_montant_ttc, 1);
}
if ($sall) {
...
if ($resql) {
    ...
    if (!empty($arrayfields['p.total_vat']['checked'])) {
        print_liste_field_titre($arrayfields['p.total_vat']['label'], $_SERVER["PHP_SELF"], 'p.tva', '', $param, 'align="right"', $sortfield, $sortorder);
    }
    if (!empty($arrayfields['p.total_ttc']['checked'])) {
        print_liste_field_titre($arrayfields['p.total_ttc']['label'], $_SERVER["PHP_SELF"], 'p.total', '', $param, 'align="right"', $sortfield, $sortorder);
    }
    if (!empty($arrayfields['u.login']['checked'])) {
...
// Extra fields
    ...
    // Show total line
    if (isset($totalarray['totalhtfield']) || isset($totalarray['totalvatfield']) || isset($totalarray['totalttcfield']) || isset($totalarray['totalamfield']) || isset($totalarray['totalrtpfield']) || isset($totalarray['totalizable'])
    ) {
