...
//if (! empty($this->date)  && ! empty($this->dateend)) $this->durationa=($this->dateend - $this->date);
        ...
        if (is_object($this->contact) && isset($this->contact->id) && $this->contact->id > 0 && !($this->contactid > 0))
            $this->contactid = $this->contact->id;  // For backward compatibility. Using this->contact->xx is deprecated
...
//		$this->fetch_userassigned();
    ...
    function fetch($id, $ref = '', $ref_ext = '')
    {
        ...
        $sql .= " WHERE ";
        if ($ref)
            $sql .= " a.id=" . $ref;           // No field ref, we use id
        elseif ($ref_ext)
...
//if ($this->date  && $this->dateend) $this->durationa=($this->dateend - $this->date);
    ...
    function load_board($user, $load_state_board = 0)
    {
        ...
        if ($resql) {
            if (empty($load_state_board)) {
                ...
                $response->url = BASE_URI . '?controller=comm/action&method=list&actioncode=0&amp;status=todo&amp;mainmenu=agenda';
                if ($user->rights->agenda->allactions->read) {
    ...
    function LibStatut($percent, $mode, $hidenastatus = 0, $datestart = '')
    {
        ...
        elseif ($mode == 6) {
            ...
            elseif ($percent == 0)
                return $langs->trans('StatusActionToDo') . ' (0%) ' . img_picto($langs->trans('StatusActionToDo'), 'statut1');
            elseif ($percent > 0 && $percent < 100)
                return $langs->trans('StatusActionInProcess') . ' (' . $percent . '%) ' . img_picto($langs->trans('StatusActionInProcess') . ' - ' . $percent . '%', 'statut3');
            elseif ($percent >= 100)
                return $langs->trans('StatusActionDone') . ' (100%) ' . img_picto($langs->trans('StatusActionDone'), 'statut6');
        }
        elseif ($mode == 7) {
            if ($percent == -1 && !$hidenastatus)
                return img_picto($langs->trans('StatusNotApplicable'), 'statut9');
    ...
    function getNomUrl($withpicto = 0, $maxlength = 0, $classname = '', $option = '', $overwritepicto = 0, $notooltip = 0, $save_lastsearch_value = -1)
    {
        ...
        $result .= $linkstart;
        if ($withpicto)
            $result .= img_object(($notooltip ? '' : $langs->trans("ShowAction") . ': ' . $libelle), ($overwritepicto ? $overwritepicto : 'action'), ($notooltip ? 'class="' . (($withpicto != 2) ? 'paddingright ' : '') . 'valigntextbottom"' : 'class="' . (($withpicto != 2) ? 'paddingright ' : '') . 'classfortooltip valigntextbottom"'), 0, 0, $notooltip ? 0 : 1);
        $result .= $libelleshort;
        $result .= $linkend;
