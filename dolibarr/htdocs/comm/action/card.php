...
if ($action == 'add') {
    ...
    if (empty($backtopage)) {
        if ($socid > 0) {
            ...
            $backtopage = BASE_URL . '?controller=societe&method=agenda&socid=' . $socid;
        } else
        // $backtopage = DOL_URL_ROOT . '/comm/action/index.php';
            $backtopage = BASE_URL . '?controller=comm/action&method=index';
    }
...
if ($action == 'create') {
    ...
    if (isset($_GET['status']) || isset($_POST['status']))
        ...
        if (GETPOST('complete') == '0' || GETPOST("afaire") == 1)
            $percent = '0';
        else if (GETPOST('complete') == 100 || GETPOST("afaire") == 2)
            $percent = 100;
    }
    ...
    if (!empty($conf->projet->enabled)) {
        ...
        print ' &nbsp; <a href="' . BASE_URI . '?controller=projet&method=card&socid=' . $societe->id . '&action=create">' . $langs->trans("AddProject") . '</a>';
    ...
    if ($action == 'edit') {
        ...
        if (!empty($conf->projet->enabled)) {
            ...
            if ($numprojet == 0) {
                ...
                print ' &nbsp; <a href="' . BASE_URI . '?controller=projet&method=card&socid=' . $object->socid . '&action=create&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?id=' . $object->id . '&action=edit') . '">' . $langs->trans("AddProject") . '</a>';
            }
            print '</td></tr>';
        }
