...
if (!$socid && ($action != 'create' && $action != 'add' && $action != 'update'))
    accessforbidden();
    ...
    if ($action == 'add') {
        ...
        if ($result >= 0) {
            if (!empty($backtopage)) {
                ...
                header('Location: ' . BASE_URI . '?controller=commande&method=contact&action=editdelivery_adress&socid=' . $socid . '&id=' . $originid);
                exit;
            } elseif ($origin == 'propal') {
                // header("Location: ../comm/propal/contact.php?action=editdelivery_adress&socid=".$socid."&id=".$originid);
                header('Location: ' . BASE_URI . '?controller=comm/propal&method=contact&action=editdelivery_adress&socid=' . $socid . '&id=' . $originid);
                exit;
            } elseif ($origin == 'shipment') {
                // header("Location: ../expedition/card.php?id=".$originid);
                header('Location: ' . BASE_URI . '?controller=expedition&method=card&id=' . $originid);
                exit;
            } else {
    ...
    // Update address
        ...
        if ($result >= 0) {
            if (!empty($backtopage)) {
                ...
                header('Location: ' . BASE_URI . '?controller=commande&method=contact&id=' . $originid);
                exit;
            } elseif ($origin == 'propal') {
                // header("Location: ../comm/propal/contact.php?id=" . $originid);
                header('Location: ' . BASE_URI . '?controller=comm/propal&method=contact&id=' . $originid);
                exit;
            } elseif ($origin == 'shipment') {
                // header("Location: ../expedition/card.php?id=" . $originid);
                header('Location: ' . BASE_URI . '?controller=expedition&method=card&id=' . $originid);
